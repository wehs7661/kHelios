...
!
! Kevin Hauser, James Maier, Mosavverul Hassan, Yiging He, Carlos Simmerling
...
!---------------------------------------------------------------------------
!
!      Open the control file. Expected program usage: ./HeliosvX.o input
!
!---------------------------------------------------------------------------
...
!----------------------------------------------------------------------------
  ...
  do iatom = 1, natoms
    write(*,*) new_xyz(iatom,1), new_xyz(iatom,2), new_xyz(iatom,3)
  end do
...
1011 format(/1x, 64('~'), /10x, &
             'Helios  execution info                              :::', &
             /1x, 64('~')/)
1012 format(/1x, 64('~'), /10x, &
             'Helios  results                                     :::', &
             /1x, 64('~')/)
1200 format(/10x, 55('~'), /10x, &
             'Helios  execution timing:                         :::', &
             /10x, 55('~')/)
